...
Full audit completed across all crates. Zero `expect()`/`unwrap()` in production source. Test files use `expect()` as assertions, which is intentional.
...
discipline, appears to have no precedent in the ephemeral-agent literature.
...
| 68 | `chronos-vdf/Cargo.toml` | n/a | `rand` was used only by `blind.rs` for `RandBigInt`; `thiserror` was never used in this crate at all, since error types come from `chronos_core::ChronosError` | Both dependencies dropped |
